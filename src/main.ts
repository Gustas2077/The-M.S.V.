...
function updateThemeClass(paletteName: string) {
	bodyEl.classList.toggle("theme-deep-red", paletteName === "deep-red");
	bodyEl.classList.toggle("theme-deep-purple", paletteName === "deep-purple");
}

updateThemeClass(currentPaletteName);
...
function deepRedColor(x: number): [number, number, number] {
	const t = clamp01(x);
	return [Math.sqrt(t), 0, 0];
}
